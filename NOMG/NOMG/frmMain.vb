...
Public Class frmMain
    ...
    Private Sub btnLogOut_Click(sender As Object, e As EventArgs) Handles btnLogOut.Click
        ...
        If Not frmAccountInformation.currentUser.GetSetInDrApp() Then
            ExportDoctorAppointments(frmAccountInformation.currentUser.GetDoctor(), frmAccountInformation.currentUser.GetListAppointments())
            frmAccountInformation.currentUser.SetSetInDrApp(True)
        End If
            ...
            If frmAccountInformation.currentUser.GetListAppointments.Count < 2 Then
                ...
                Do While dteTracker <= frmAccountInformation.currentUser.GetDteLMC.AddMonths(9)
                    ...
                    ElseIf dteTracker <= frmAccountInformation.currentUser.GetDteLMC.AddMonths(6) Then
                        ...
                        intI = 0
                        Do While intI < frmAccountInformation.currentUser.GetDoctor.listDrAppointments.Count
                            If frmAccountInformation.currentUser.GetDoctor.listDrAppointments(intI) = dteTracker.AddDays(20) Then
                                counter = counter + 1
                            End If
                            intI = intI + 1
                        Loop
                    ...
                    Else
                        ...
                        intI = 0
                        Do While intI < frmAccountInformation.currentUser.GetDoctor.listDrAppointments.Count
                            If frmAccountInformation.currentUser.GetDoctor.listDrAppointments(intI) = dteTracker.AddDays(10) Then
                                counter = counter + 1
                            End If
                            intI = intI + 1
                        Loop
    ...
    Public Sub ExportFileData()
        ...
        Try
            ...
            Using file As New FileStream(filePath, FileMode.Create, FileAccess.Write)
                ...
                ' Write to file
                Dim bytesToWrite() As Byte = Encoding.UTF8.GetBytes(userInfo + textToWrite2 + vbCrLf + textToWrite3 + vbCrLf + textToWrite4 + vbCrLf + doctorInfo + vbCrLf + textToWrite7 + vbCrLf + hadDrAppointment)
        ...
        ' DOCTOR DATA
        For Each doctor In frmAccountInformation.listDoctors
            Try
                Dim filePath As String = doctor.GetName() + ".txt"
                Using file As New FileStream(filePath, FileMode.Create, FileAccess.Write)

                    Dim textToWrite2 As String = String.Empty
                    textToWrite2 += "Doctor Name: " + doctor.GetName()
                    textToWrite2 += vbCrLf ' Add an extra line break after the list of appointments

                    Dim userInfo As String = String.Empty
                    If doctor.listDrAppointments.Count <= 0 Then
    ...
    Public Sub ExportDoctorAppointments(doctor As frmAccountInformation.Doctor, appointments As List(Of Date))
        Try
            Dim filePath As String = doctor.GetName() + ".txt"
            Using file As New FileStream(filePath, FileMode.Append, FileAccess.Write)
                ...
                If appointments.Count > 0 Then
                    textToWrite += "New Patient Appointments:" + vbCrLf
                    For Each appointment In appointments
                        textToWrite += appointment.ToString() + vbCrLf
                    Next
                    textToWrite += vbCrLf ' Add an extra line break after the list of appointments
                End If
        ...
        Catch ex As Exception
            ' Handle any exceptions (e.g., file access, permissions, etc.)
            MsgBox("Error: Can't export doctor appointments. " & ex.Message)
        End Try
    End Sub
